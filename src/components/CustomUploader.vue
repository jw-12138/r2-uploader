  ...
  <div>
    <form action="javascript:">
      ...
      <div class="mt-2" v-show="fileList.length">
        <button class="inline-block w-auto" @click="upload" :disabled="uploading">🔥 Upload</button>
      </div>
      <div>
        ...
        <div v-show="fileList.length + uploadedList.length > 0">
          <div class="text-center text-xs py-4" v-show="uploading || uploadedList.length > 0">
            Uploading at <span class="dark:text-green-200 text-green-800 italic font-bold">{{ globalSpeed }}</span><span
        ...
        <div class="mb-2" v-show="fileList.length > 0 && !uploading">
          <input :disabled="uploading" class="text-xs" type="checkbox" id="skip_uploading_if_filename_is_the_same"
                 v-model="skipFilesWithTheSameName"> <label
          for="skip_uploading_if_filename_is_the_same" class="text-xs">Skip uploading
          files with the same name</label>
        </div>
          ...
          <div
            ...
            <div v-show="editKey === item.key" class="flex">
              <form action="javascript:" @submit="renameThisFile(item)" class="flex mb-0 w-full">
                <input class="text-xs w-full" type="text" style="padding: .2rem .4rem; margin: 0; height: auto"
                       :value="item.key" :id="'input_' + item.key">
                <button
            ...
            <span v-show="editKey !== item.key" class="inline-block break-all" @click="showRenameInput(item.key)">{{
                item.key
              }}</span><br><span :style="{
            ...
            }">{{ parseByteSize(item.size) }} <span
            v-show="uploading"> / {{ progressMap[item.key] }}%</span></span>
          </div>
...
window.onbeforeunload = uploading.value ? function () {
  return true
} : undefined
...
watch(skipFilesWithTheSameName, (val) => {
  if (val) {
    ...
    fileList.value.forEach(file => {
      let match = uploadedFiles.findIndex(el => el.key === formatFileName(file.key))
...
function uploadFile(file) {
  ...
  let fileName = '/' + formatFileName(file.key)
  if (endPoint[endPoint.length - 1] === '/') {
    fileName = formatFileName(file.key)
  }
